...
object Application {
  ...
  def main(args: Array[String]) {
    ...
    def parseFlight(str: String): Option[Flight] = {
      val line = str.split(",")
      try {
        Some(Flight(line(0), line(1), line(2), line(3), line(4).toInt, line(5), line(6), line(7), line(8), line(9).toDouble, line(10).toDouble, line(11).toDouble, line(12).toDouble, line(13).toDouble, line(14).toDouble, line(15).toDouble, line(16).toInt))
      } catch {
        case e: Exception =>
          line.map { l =>
            println(l)
    ...
    val textRDD = sc.textFile("/Users/debojit/code_bases/spark-mlib-project/data/rita2014jan.csv")

    val flightsRDD = textRDD.flatMap(txt => parseFlight(txt)).cache()
